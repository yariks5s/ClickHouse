...
std::set<String> fetchmongodbTablesList(T & tx, const String & mongodb_schema)
{
    ...
    if (schemas.size() <= 1)
    {
        std::string query = fmt::format(
            ...
            mongodb_schema.empty() ? quoteString("public") : quoteString(mongodb_schema));
...
static DataTypePtr convertmongodbDataType(String & type, Fn<void()> auto && recheck_array, bool is_nullable = false, uint16_t dimensions = 0)
{
    ...
    if (is_array)
    {
        ...
        /// (it might return incorrect 0 number, for example, when a mongodb table is created via 'as select * from table_with_arrays').
    ...
    T & tx, const String & mongodb_table, const String & query, bool use_nulls, bool only_names_and_types)
{
    ...
    try
    {
        std::set<size_t> recheck_arrays_indexes;
        {
            ...
            if (only_names_and_types)
            {
                ...
                while (stream >> row)
                {
                    columns.push_back(NameAndTypePair(std::get<0>(row), convertmongodbDataType(std::get<1>(row), recheck_array)));
                    ++i;
                }
            }
            else
            {
                ...
                while (stream >> row)
                {
                    auto data_type = convertmongodbDataType(
        ...
        for (const auto & i : recheck_arrays_indexes)
        {
            ...
            pqxx::result result{tx.exec(fmt::format("SELECT array_ndims({}) FROM {} LIMIT 1", name_and_type.name, mongodb_table))};
    ...
    {
        throw Exception(ErrorCodes::UNKNOWN_TABLE, "mongodb table {} does not exist", mongodb_table);
    }
    ...
    {
        e.addMessage("while fetching mongodb table structure");
        throw;
    }

    return !columns.empty()
        ? std::make_shared<mongodbTableStructure::ColumnsInfo>(NamesAndTypesList(columns.begin(), columns.end()), std::move(attributes))
        : nullptr;
}
        ...
        T & tx, const String & mongodb_table, const String & mongodb_schema, bool use_nulls, bool with_primary_key, bool with_replica_identity_index)
{
    ...
    if (mongodb_schema.empty())
        ...
        where += fmt::format(" AND relnamespace = (SELECT oid FROM pg_namespace WHERE nspname = {})", quoteString(mongodb_schema));
    ...
    auto mongodb_table_with_schema = mongodb_schema.empty() ? mongodb_table : doubleQuoteString(mongodb_schema) + '.' + doubleQuoteString(mongodb_table);
    table.physical_columns = readNamesAndTypesList(tx, mongodb_table_with_schema, query, use_nulls, false);

    if (!table.physical_columns)
        throw Exception(ErrorCodes::UNKNOWN_TABLE, "mongodb table {} does not exist", mongodb_table_with_schema);

    if (with_primary_key)
    {
        /// wiki.mongodb.org/wiki/Retrieve_primary_key_columns
        query = fmt::format(
        ...
        table.primary_key_columns = readNamesAndTypesList(tx, mongodb_table_with_schema, query, use_nulls, true);
    }
    ...
    if (with_replica_identity_index && !table.primary_key_columns)
    {
        query = fmt::format(
            ...
            quoteString(mongodb_table),
            (mongodb_schema.empty() ? quoteString("public") : quoteString(mongodb_schema))
        );

        table.replica_identity_columns = readNamesAndTypesList(tx, mongodb_table_with_schema, query, use_nulls, true);
    }
